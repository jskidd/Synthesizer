...
const keys = {
  ...
  O: { element: getElementByNote("C2#"), note: "C#", octaveOffset: 1 },
  L: { element: getElementByNote("D2"), note: "D", octaveOffset: 1 },
  P: { element: getElementByNote("D2#"), note: "D#", octaveOffset: 1 },
...
const playKey = (key) => {
  ...
  noteGainNode.connect(audioCtx.destination);
  ...
  const setAttack = () => {
    noteGainNode.gain.exponentialRampToValueAtTime(maxGain, audioCtx.currentTime + 0.01);
  };

  const setDecay = () => {
    noteGainNode.gain.exponentialRampToValueAtTime(sustainedGain, audioCtx.currentTime + 1);
  };

  const setRelease = () => {
    noteGainNode.gain.exponentialRampToValueAtTime(zeroGain, audioCtx.currentTime + 2);
  };
